# Prompt: Plan de implementación — EPIC 2: Sistema de roles y permisos

> **Uso:** Copia el bloque `PROMPT` y reemplaza **únicamente** `{{US_ID}}` por el
> identificador de la US a implementar (ej. `US-2.1`, `US-2.2`, `US-2.3`, `US-2.4`).
>
> **No hay más variables que cambiar.** Todo el contexto técnico del EPIC 2 está
> embebido en el prompt para que el plan sea preciso desde la primera ejecución.
...
**{{US_ID}}** del EPIC 2 — Sistema de roles y permisos.
...
Genera un test por cada CA definido en `US-EPIC2.md` para **{{US_ID}}**.
...
Para **cada CA de {{US_ID}}**, proporciona el comando `curl` exacto que lo verifica.
...
- No generes código que no sea estrictamente necesario para **{{US_ID}}**
...
Genera el plan completo para **{{US_ID}}**.
```
...
2. Reemplaza **todas** las ocurrencias de `{{US_ID}}` por el identificador real (ej. `US-2.3`)
3. Pega el prompt en una nueva conversación de Claude Code desde la raíz del proyecto

## Variables a personalizar por US

| Variable | Descripción | Valores válidos |
|---|---|---|
| `{{US_ID}}` | Identificador de la User Story | `US-2.1` · `US-2.2` · `US-2.3` · `US-2.4` |

> Solo esta variable cambia entre ejecuciones.

## Skills activados en el prompt

| Skill | Cuándo se activa |
|---|---|
| `/clean-code` | En cada cambio de código propuesto |
| `/backend-testing` | Al generar tests de Spring Boot (roles, 401 vs 403) |
| `/java-refactoring-extract-method` | Si lógica de asignación de rol supera 10 líneas |
| `/postgresql-code-review` | Al diseñar la columna `role` en la tabla `users` |
| `/vercel-react-best-practices` | En componentes React con control de visibilidad por rol |
| `/code-refactoring` | Revisión de duplicación del predicado de rol antes de cerrar |
| `/simplify` | Revisión final de calidad y eficiencia del código añadido |

## Por qué este template es distinto al genérico `impl-plan-us.md`

| Aspecto | `impl-plan-us.md` | Este template |
|---|---|---|
| Contexto técnico | Genérico | Estado exacto del código (líneas concretas, bugs conocidos) |
| Tests de seguridad | Patrón genérico | Matriz 401/403/2xx + `@WithMockUser` + escenarios de rol |
| Dependencia crítica | Mención genérica | Bloqueo explícito: US-2.1 antes de US-2.3 |
| Verificación curl | Template genérico | Script completo con captura de token ADMIN y USER |
| DoD | Genérico | Checklist específico de seguridad por roles |
| Riesgo principal | No mencionado | `new ArrayList<>()` en `UserDetailsServiceImpl` identificado |
