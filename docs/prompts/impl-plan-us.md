# Prompt: Plan de implementación para una User Story

> **Uso:** Copia este prompt y reemplaza `{{US_ID}}` por el identificador de la US que quieres implementar (ej. `US-1.1`, `US-2.3`, `US-4.1`).
> El fichero de US's para el EPIC correspondiente sigue el patrón `US-EPIC<N>.md`.
...
Voy a pedirte que generes un **plan de implementación detallado** para la User Story **{{US_ID}}**.
...
- `US-EPIC<N>.md` — definición completa de la US, criterios de aceptación y DoD del epic
...
1. Reemplaza todas las ocurrencias de `{{US_ID}}` por el identificador real (ej. `US-1.1`)
2. Reemplaza `US-EPIC<N>.md` por el fichero correcto (ej. `US-EPIC1.md` para US del EPIC 1)
3. Pega el prompt en una nueva conversación de Claude Code desde la raíz del proyecto
4. Claude leerá el código real antes de generar el plan

## Skills utilizados en el prompt

| Skill | Cuándo se activa |
|-------|-----------------|
| `/clean-code` | En cada cambio de código propuesto |
| `/backend-testing` | Al generar tests de Spring Boot |
| `/java-refactoring-extract-method` | Al extraer métodos en Java |
| `/postgresql-code-review` | Si hay queries o diseño de esquema |
| `/vercel-react-best-practices` | Si hay componentes React nuevos/modificados |
| `/code-refactoring` | Revisión final antes de cerrar la US |
